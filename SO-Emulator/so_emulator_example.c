...
static const uint16_t code_mov[MEM_SIZE] = {
  0x4000 + 0x100 * 0 + 1,           // MOVI A, 1             A := 1
  0x4000 + 0x100 * 1 + 3,           // MOVI D, 3             D := 3
  0x4000 + 0x100 * 2 + 0x11,        // MOVI X, 0x11          X := 17
  0x4000 + 0x100 * 3 + 0x21,        // MOVI Y, 0x21          Y:= 33
  0x0000 + 0x100 * 4 + 0x0800 * 0,  // MOV  [X], A           [X] = [17] := 1
  0x0000 + 0x100 * 5 + 0x0800 * 1,  // MOV  [Y], D           [Y] = [33] := 3
  0x4000 + 0x100 * 6 + 0x07,        // MOVI [X + D], 0x07    [X+D] = [20] := 7, [Y+D] = [36] = 0 ?
  0x0004 + 0x100 * 1 + 0x0800 * 0,  // ADD  D, A             D += 1, D = 4
  0x4000 + 0x100 * 6 + 0x08,        // MOVI [X + D], 0x08    [X+D] = [21] = 8
  0x0000 + 0x100 * 7 + 0x0800 * 6,  // MOV  [Y + D], [X + D] [Y+D] = [37] := [X+D] = 8
